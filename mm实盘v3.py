...
class Config:
    ...
    ORDER_REFRESH_SEC = 300  # 订单刷新时间
    SYMBOL = "BTCUSDT"  # 交易对
    ATR_PERIOD = 14  # ATR计算周期
    BASE_SPREAD_PCT = Decimal("0.03")  # 基础价差百分比
    ATR_MULTIPLIER = Decimal("1.5")  # 波动系数
    MAX_SPREAD_PCT = Decimal("0.1")  # 最大允许价差
    MIN_SPREAD_PCT = Decimal("0.02")  # 最小允许价差
...
class EnhancedBinanceClient:
    ...
    def request(self, endpoint, params=None):
        ...
        try:
            ...
            # 在deleteOpenOrders请求中强制添加symbol
            if endpoint == "deleteOpenOrders":
                params.setdefault("symbol", Config.SYMBOL)
...
class EnhancedMarketMaker:
    def __init__(self, client, symbol):
        ...
        self.atr_value = None
        self.price_history = deque(maxlen=Config.ATR_PERIOD * 2)
        self.start_atr_updater()
    ...
    def calculate_atr(self):
        ...
        try:
            if not self.price_history:
                ...
                response = self.client.request("getKlines", params)
                if not isinstance(response, list) or len(response) < Config.ATR_PERIOD:
            ...
            if len(true_ranges) >= Config.ATR_PERIOD:
                self.atr_value = (
                    sum(true_ranges[: Config.ATR_PERIOD]) / Config.ATR_PERIOD
                )
                for tr in true_ranges[Config.ATR_PERIOD :]:
                    self.atr_value = (
                        self.atr_value * (Config.ATR_PERIOD - 1) + tr
                    ) / Config.ATR_PERIOD
        except Exception as e:
            self.logger.error(f"ATR计算失败: {str(e)}")

    def check_orderbook_depth(self, min_depth=Decimal("0.1")):
        """检查买卖盘前5档总深度（调整为0.1 BTC）"""
        bid_depth = sum(float(bid[1]) for bid in self.orderbook["bids"][:5])
        ask_depth = sum(float(ask[1]) for ask in self.orderbook["asks"][:5])
        self.logger.debug(
            f"深度检查: 买盘{bid_depth:.4f} 卖盘{ask_depth:.4f} (阈值{min_depth})"
        )
        return bid_depth >= float(min_depth) and ask_depth >= float(min_depth)

    def get_dynamic_spread(self, mid_price):
        """动态调整价差（增加异常处理）"""
        try:
            if self.atr_value is None or mid_price is None:
                return Config.BASE_SPREAD_PCT

            atr_ratio = self.atr_value / mid_price
            dynamic_spread = Config.BASE_SPREAD_PCT + (
                atr_ratio * Config.ATR_MULTIPLIER
            )
            return max(
                min(dynamic_spread, Config.MAX_SPREAD_PCT), Config.MIN_SPREAD_PCT
            )
        except Exception as e:
            self.logger.error(f"价差计算异常: {str(e)}，使用默认价差")
            return Config.BASE_SPREAD_PCT
    ...
    def generate_orders(self):
        ...
        # 新增流动性检查
        if not self.check_orderbook_depth():
            self.logger.warning("订单簿深度不足，暂停挂单")
            return []

        # 使用动态价差
        spread_pct = self.get_dynamic_spread(mid_price)
        buy_price = mid_price * (1 - spread_pct)
        sell_price = mid_price * (1 + spread_pct)

        # 价格有效性验证
        if (
            buy_price <= self.min_price
            or sell_price <= self.min_price
            or buy_price >= sell_price
        ):
            self.logger.error(f"价格异常: 买入{buy_price} 卖出{sell_price}")
            return []
    ...
    def log_order_details(self, mid, buy_price, sell_price, buy_qty, sell_qty):
        ...
        self.logger.info(
            f"ATR指标: {self.atr_value:.4f} ({self.atr_value/mid*100:.2f}%)"
        )
        self.logger.info(f"动态价差范围: ±{spread_pct*100:.2f}%")
        self.logger.info("=" * 40 + "\n")
    ...
    def place_orders(self, orders):
        ...
        for side, qty, price in orders:
            # 新增参数校验
            if price <= 0 or qty <= 0:
                self.logger.error(f"无效订单参数: {side} {qty}@{price}")
                continue
            try:
